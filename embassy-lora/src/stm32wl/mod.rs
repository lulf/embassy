...
use embassy_stm32::{
    ...
    subghz::{
        ...
        RfFreq, SpreadingFactor as SF, StandbyClk, SubGhz, TcxoMode, TcxoTrim, Timeout, TxParams,
    },
};
...
pub struct SubGhzRadio<'a, Tx, Rx>
where
    Tx: TxDmaChannel<SUBGHZSPI>,
    Rx: RxDmaChannel<SUBGHZSPI>,
{
    radio: SubGhz<'a, Tx, Rx>,
    switch: RadioSwitch<'a>,
}

impl<'a, Tx, Rx> SubGhzRadio<'a, Tx, Rx>
where
    Tx: TxDmaChannel<SUBGHZSPI>,
    Rx: RxDmaChannel<SUBGHZSPI>,
{
    /// Create a new instance of a SubGhz radio for LoRaWAN.
    pub fn new(radio: SubGhz<'a, Tx, Rx>, switch: RadioSwitch<'a>) -> Self {
        Self { radio, switch }
    }
    ...
    pub(crate) async fn configure(&mut self) -> Result<(), RadioError> {
        ...
        self.radio.set_pa_ocp(Ocp::Max140m).await?;
        ...
        self.radio.set_lora_sync_word(LoRaSyncWord::Public).await?;
    ...
    async fn do_tx(&mut self, config: TxConfig, buf: &[u8]) -> Result<u32, RadioError> {
        ...
        let mod_params = LoRaModParams::new()
            ...
            .set_ldro_en(true);
        self.radio.set_lora_mod_params(&mod_params).await?;
        ...
        self.radio.set_lora_packet_params(&packet_params).await?;

        let irq_cfg = CfgIrq::new()
            .irq_enable_all(Irq::TxDone)
            .irq_enable_all(Irq::RxDone)
            .irq_enable_all(Irq::Timeout);
        self.radio.set_irq_cfg(&irq_cfg).await?;

        self.radio.set_buffer_base_address(0, 0).await?;
        self.radio.write_buffer(0, buf).await?;

        self.radio.set_tx(Timeout::DISABLED).await?;

        loop {
            info!("Awaiting RADIO IRQ");
            let (_status, irq_status) = self.radio.wait_irq().await?;

            if irq_status & Irq::PreambleDetected.mask() != 0 {
                info!("Preamble detected, ignoring");
            } else if irq_status & Irq::TxDone.mask() != 0 {
                let stats = self.radio.lora_stats().await?;
                let (status, error_mask) = self.radio.op_error().await?;
                info!(
                    "TX done. Stats: {:?}. OP error: {:?}, mask {:?}",
                    stats, status, error_mask
                );
    ...
    async fn do_rx(
        ...
        self.radio.set_rx(Timeout::DISABLED).await?;
        info!("RX started");

        loop {
            let (status, irq_status) = self.radio.wait_irq().await?;
            info!("RX IRQ {:?}, {:?}", status, irq_status);
            if irq_status & Irq::PreambleDetected.mask() != 0 {
                info!("Preamble detected, ignoring");
            } else if irq_status & Irq::RxDone.mask() != 0 {
                let (status, len, ptr) = self.radio.rx_buffer_status().await?;

                let packet_status = self.radio.lora_packet_status().await?;
                ...
                self.radio
                    .read_buffer(ptr, &mut buf[..len as usize])
                    .await?;
                self.radio.set_standby(StandbyClk::Rc).await?;
...
impl<Tx, Rx> PhyRxTx for SubGhzRadio<'static, Tx, Rx>
where
    Tx: TxDmaChannel<SUBGHZSPI> + 'static,
    Rx: RxDmaChannel<SUBGHZSPI> + 'static,
{
    type PhyError = RadioError;

    #[rustfmt::skip]
    type TxFuture<'m> where Tx: 'm, Rx: 'm = impl Future<Output = Result<u32, Self::PhyError>> + 'm;
    fn tx<'m>(&'m mut self, config: TxConfig, buf: &'m [u8]) -> Self::TxFuture<'m> {
        self.do_tx(config, buf)
    }

    #[rustfmt::skip]
    type RxFuture<'m> where Tx: 'm, Rx: 'm = impl Future<Output = Result<(usize, RxQuality), Self::PhyError>> + 'm;
    fn rx<'m>(&'m mut self, config: RfConfig, buf: &'m mut [u8]) -> Self::RxFuture<'m> {
        self.do_rx(config, buf)
    }
}

impl<'a, Tx, Rx> Timings for SubGhzRadio<'a, Tx, Rx>
where
    Tx: TxDmaChannel<SUBGHZSPI>,
    Rx: RxDmaChannel<SUBGHZSPI>,
{
