...
use embassy::traits::{gpio::WaitForRisingEdge, spi::FullDuplex};
    ...
    RFS: RadioSwitch + 'static,
{
    ...
    fn tx<'m>(&'m mut self, config: TxConfig, buf: &'m [u8]) -> Self::TxFuture<'m> {
        ...
        async move {
            ...
            let i = self.irq.wait_for_rising_edge();

            trace!("TRANSMIT PAYLOAD");

            self.radio.transmit_payload(buf).await?;

            trace!("TX wait for IRQ");
            i.await;
            trace!("IRQ RAISED!");
